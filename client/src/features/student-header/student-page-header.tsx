...
const StudentPageHeader = ({ student }: { student: Student }) => {
  ...
  const subpages: Array<SubpageData> = [
    { name: 'Dados cadastrais', route: 'profile', active: false },
  ...
  return (
    <div className="">
      <div className="flex items-center gap-10 border-b bg-[#f7f7f7] px-10 py-4 dark:bg-[#0c0c0c]">
        <AvatarEditable
          name={student.name}
        ...
        <div className="flex flex-col gap-4">
          ...
          <ul className="mt-3 flex h-5 items-center space-x-4">
            {subpages.map((subpage, index) => (
              <Fragment key={subpage.route}>
                <li className="hover:underline hover:decoration-2">
                  <Link
                    className={subpage.active ? 'underline decoration-2' : ''}
                    href={`/students/${username}/${subpage.route}`}
                  >
                    {subpage.name}
                  </Link>
                </li>
